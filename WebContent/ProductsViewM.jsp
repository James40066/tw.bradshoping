...
<%
	...
	//預先查詢商品總筆數(可以簡化成sql的ston procedure)
	String sql1 = "SELECT count(*) FROM product where isSale = 1";
	...
	String sql2 = "SELECT product.pid,product.pname,product.pprice,product.isSale,productphoto.defaultPic,productphoto.big,product.special"
			+ " FROM cart.product left join cart.productphoto on cart.product.pid =cart.productphoto.pid where isSale =1 and defaultPic=1  order by pid limit "
			+ ((PageNum - 1) * ProductMaxSize) + "," + ProductMaxSize;
	...
	//http://localhost:8080/tw.bradshoping/productShow.jsp?product=68
